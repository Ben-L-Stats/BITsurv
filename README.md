...
The examples from the paper are available in the 'Examples' folder. The simulation from the paper are available in the 'Simulation' folder.
...
To use BITsurv from now on, just run the following:
```
library(dplyr)
library(BITsurv)
```
